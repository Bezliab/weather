...
class Particle {
  ...
  reset() {
    ...
    this.size = Math.random() * (this.type === 'rain' ? 1.5 : 4) + (this.type === 'rain' ? 0.5 : 1);
    this.speed = Math.random() * (this.type === 'rain' ? 14 : (this.type === 'snow' ? 1.2 : 2)) + (this.type === 'rain' ? 8 : 0.5);
  ...
  update() {
    ...
    if (this.y > canvas.height + 20 || this.x < -10 || this.x > canvas.width + 10) {
      this.reset();
    }
  }
  ...
  draw() {
    ...
    if (this.type === 'rain') {
      ...
      const gradient = ctx.createRadialGradient(this.x, this.y, 0, this.x, this.y, this.size * 18);
